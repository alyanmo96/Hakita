 ...
 */
  ...
  {// connect with DB to save the new data
    $con=mysqli_connect("Localhost","id11176973_haki1","haki321","id11176973_haki");
      ...
      if ($_POST['last_name']){// if ADMIN update user last name
        ...
        $upDate="UPDATE `teachers` SET `lname`='$lName'WHERE id=$AdminPutId";//update it on table
        $results = mysqli_query($con,$upDate);
      }        
      if ($_POST['price']){// if ADMIN update user price
        $Price=$_POST['price'];
        $upDate="UPDATE `teachers` SET `price`='$Price'WHERE id=$AdminPutId";//update it on table
        $results = mysqli_query($con,$upDate);
      }        
      if ($_POST['status']){// if ADMIN update user status
        $status=$_POST['status'];
        $upDate="UPDATE `teachers` SET `status`='$status'WHERE id=$AdminPutId";//update it on table
        $results = mysqli_query($con,$upDate);
      }
      if ($_POST['email']){  // if ADMIN update user email
        $Email=$_POST['email'];
        $upDate="UPDATE `teachers` SET `email`='$Email'WHERE id=$AdminPutId";//update it on table
        $results = mysqli_query($con,$upDate);
      }        
      if ($_POST['password']){ // if ADMIN update user password
        ...
        if ($_POST['password']==$_POST['verifyPassword']){//next variables to check if ADMIN insert letters/number on password
            ...
            if($invalidPass==-1){//if the password is valid
                ...
                $upDate="UPDATE `teachers` SET `password`='$pass'WHERE id=$ID";//update it on table
                $results = mysqli_query($con,$upDate);
           }
        }else{//alert wrong input
      ...
      if ($_POST['phone']){// if ADMIN update user phone
          $phone=$_POST['phone'];
          $upDate="UPDATE `teachers` SET `phone`='$phone'WHERE id=$AdminPutId";//update it on table
          $results = mysqli_query($con,$upDate);
      }  
  ...
  if(isset($_POST['deleteUserButton'])){//ADMIN going to delete the choosen user
    ...
    $con=mysqli_connect("Localhost","id11176973_haki1","haki321","id11176973_haki");
    $sql = "DELETE FROM teachers WHERE id=$deleteUserId ";//update it on table
  ...
  <body>
    ...
    <div class="container bootstrap snippet">
      ...
      <div class="row">
        <div><!--left col-->                
          <div class="col-sm-9">
            <ul class="nav nav-tabs"><li class="active"><h1><?php echo $username ?></h1></li><!--print username of user--></ul>              
