 ...
 */
    ...
    //insert into show this teacher profile table    
    addToProfileViewerCounter($IDOfTeacher);
    ...
    if(isset($_POST['courseClassLesson'])||isset($_POST['chooseLesson'])){    
        $courseName=$_POST['courseClassLesson'];
        if($courseName=="other"){
            $courseName="אחר";
        }
        $dashboardSection=1;
        $lessonTime=$_POST['chooseLesson'];
        ...
        $lastDayThisMonth = date("t", strtotime($firstday));
        ...
        while ($scheduleRow=mysqli_fetch_assoc($scheduleResult)){//save the data of asking lesson on DB
                if($scheduleRow['idOfTeacher']==$IDOfTeacher && $scheduleRow['dayOfLesson']==$day && $scheduleRow['hourOFLesson']==$hour){
                    ...
                    $lesson_date=$scheduleRow['lessonDate'];
                    ...
                    //send a message automaticly from student to teacher
                    $message_text='הודעה אוטומתית, שלום רב, אני מתעניין/נת לקחת איתך שיעור בנושא של- '.$courseName.'. בתאריך ה-'.$lesson_date.'';
                    $query="INSERT INTO `messages`(`message_sender`,`message_receive`,`message_text`,`message_date`) VALUES
        ...
        <?php
            if(isset($_POST['chooseLessonButton'])||$_POST['chooseLesson']){//ask for a lesson with teacher 
                ...
                if($_POST['openModel']){//user after choose a lesson, automaticly will open a window with data about teacher and about lesson, to be sure.
                    ...
                    //next script for showing the window
                    if(!$IDOfUser){
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <h4 class="modal-title"> נא לבחור באיזה נושה התעניינת על מנת לקבוע</h4>
                            <?php
                                $lastDayThisMonth = date("t", strtotime($firstday));
                                if($day>$lastDayThisMonth){
                                    $day-=$lastDayThisMonth;
                                ...
                                for($ci=0;$ci<count($arrayOfTeacherCourses);$ci++){
                                  echo'<button type="submit"  value='.$arrayOfTeacherCourses[$ci].'  name="courseClassLesson" class="btn btn-primary">'.$arrayOfTeacherCourses[$ci].'</button>';
                                }
                                echo'<button type="submit"  value="other"  name="courseClassLesson" class="btn btn-primary">אחר</button>';
                                echo "<br>";
                                ...
                                echo"<input name=\"chooseLesson\" type=\"hidden\" value=\"$chooseLessonButton\" id=\"$chooseLessonButton\">";
                            ?>                                    
            ...
            <div class="container">
                <div class="span3 well">
                    ...
                    <?php
                        ...
                        echo"<h5>".teacherStudy($IDOfTeacher)."</h5>";
            ...
            <section class="board"><!--display the time board if the teacher alredy choose to display it on student side-->         
                <table class="table table-sm">
                    ...
                    <?php 
                        ...
                        if($today=="Sunday"||$today=="Monday"||$today=="Tuesday"||$today=="Wednesday"||$today=="Thursday"||$today=="Friday"||$today=="Saturday"){
                        ...
                        if($todayDate>$firstday){
                            $firstday = date('Y-m-d', strtotime("0 week"));   
                        }    
                   ...
                   <form action="viewTeacherProfile.php" method="post">                               
                        <?php
                                ...
                                while($lessonRow=mysqli_fetch_assoc($lessonResult)){
                                    if($lessonRow['idOfTeacher']==$IDOfTeacher){   
